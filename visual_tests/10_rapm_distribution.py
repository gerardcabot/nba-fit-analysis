...
def main() -> int:
    ...
    net = impact.rapm.net_rapm
    low = impact.rapm.low_sample_flag

    apply_plot_style()
    fig, ax = plt.subplots(figsize=(8, 5))
    ax.hist(net[~low], bins=20, alpha=0.75, color="#2a6f97", label="stable sample")
    if low.any():
    ...
    ax.set_title(f"Player RAPM distribution — {season} (synthetic)")
    ax.legend()
    fig.tight_layout()
    save_figure(fig, "10_rapm_distribution.png")
